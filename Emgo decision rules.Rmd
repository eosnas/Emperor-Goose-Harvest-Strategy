...
Now let use the utility function that was elicited from Eric Tayor on 27 April 2016.  
...
```
Puting both utility function together and weighting them equally gives:
...
Now turn the simulation into a function that returns expected utility so that we can use a smarter optimization tool than brut force.  
```{r eval=FALSE}
eutil <- function(Thresholds = c("Red"=20000, "Yellow"=30000), maxT = 10, iters = 100, 
                  Nobs1 = 32550, data=list(NA)){
  with(data, {
    #define harvest thresholds
    ...
    Results = list(N=array(NA, dim=c(iters,T)), 
               N.obs = array(NA, dim=c(iters,T)), 
               HarPol = array(NA, dim=c(iters,T)),
               Har = array(NA, dim=c(iters,T)),
               Utility = array(NA, dim=c(iters,T))
               )

    for(i in 1:iters){
      N <- N.obs<- numeric()
      N[1] <- N0[i]
      N.obs[1] <- Nobs1 #Sum_TOT from 2014 #rnorm(1,N[1], cv.obs*N[1])
    ...
    e.util <- mean(apply(Results$Utility,1,sum))
    return(e.util)
  })
}
...
eutil(data=dat, maxT=100, iters=Reps)
```